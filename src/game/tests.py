...
class GameBoardModelTest(TestCase):
    ...
    def test_create_invalid_json_too_many_tiles_for_ship1(self):
        ...
        try:
            ...
            self.assertEqual(e.field, "ship1", "Should catch invalid value on ship1 of board1")
    ...
    def test_create_invalid_json_too_few_tiles_for_ship2(self):
        ...
        try:
            ...
            self.assertEqual(e.field, "ship2", "Should catch invalid value on ship1 of board2")
    ...
    def test_create_too_many_ships(self):
        json_str = """{
            ...
            "ship3": ["F1", "F2", "F3"],
            "ship4": ["G1", "G2", "G3", "G4"],
            "ship5": ["H1", "H2", "H3", "H4", "H5"],
            "ship6": ["H1", "H2", "H3", "H4", "H5", "H6"]
        }"""

        state = GameBoard(json.loads(json_str))
        for i in range(1, 6):
            self.assertEqual(len(getattr(state, f'ship{i}')), i, 'each ship should have correct amount of ships')
    ...
    def test_create_gameboard_min_ships(self):
        ...
        for i in range(1, 6):
            if i == 1:
                ...
                self.assertIsNone(getattr(state, f'ship{i}'))
    ...
    def test_create_invalid_gameboard_spaced_out_ships(self):
        ...
        try:
            ...
            self.assertEqual(e.field, 'ship3', 'no weird ships allowed')
...
class GameStateModelTest(TestCase):
    def test_create_state_from_json(self):
        ...
        self.assertEqual(state.p1board.ship1[0], 'C1', 'should match json')
        response = state.recordP1play('D1')
        self.assertEqual(response.hit_tile_ids, ['D1'], 'D1 was hit')
        self.assertEqual(state.getP1state().hit_tile_ids, ['D1'], 'D1 was hit')
    ...
    def test_get_state(self):
        ...
        self.assertEqual(state.getP1state().hit_tile_ids, ['D1'], 'D1 was hit')
        self.assertEqual(state.getP1state().miss_tile_ids, ['A1'], 'A1 was miss')
